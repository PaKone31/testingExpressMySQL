...
// Retrieve frequent Articles from the database.
exports.findFrequent = (req, res) => {
  Article.getFrequent(req.params.howMany, (err, data) => {
    if (err)
      res.status(500).send({
        message: err.message || "Some error occurred while retrieving articles."
      });
...
exports.deleteAll = (req, res) => {
  Article.removeAll((err, data) => {
    if (err)
      ...
      message: `All Articles were deleted successfully!`
    });
  });
};
